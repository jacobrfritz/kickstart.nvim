...
vim.api.nvim_create_autocmd('InsertCharPre', {
  ...
  callback = function()
    ...
    if all_filled then
      -- Fire and forget the update to the FastAPI backend using a raw non-blocking TCP socket.
      -- This avoids the heavy process spawning overhead of curl on Windows on every keystroke.
      local payload =
      ...
      if client then
        client:connect('127.0.0.1', 8000, function(err)
          if not err then
            client:write(req, function()
              pcall(function()
                client:shutdown()
                client:close()
              end)
